...
WELCOME_TEXT = (
    "AI Molecule Design Assistant is a molecular AI and cheminformatics "
    "workflow for molecules generated by generative AI models. It supports "
    "general molecule-processing demos and a separate ADORA2A / xanthine "
    "target-specific structural demo with illustrative docking values. It helps "
    "move from raw generated SMILES to interpretable candidate tables by "
    "checking chemical validity, public identity, public-database "
    "differentiation signals, descriptor-based ADMET triage, chemical-space "
    "position, semantic biomedical evidence similarity, patent/IP-context "
    "evidence triage, and target-aware translational-positioning outputs. The "
    "IP-potential score is only a computational triage signal and is not a "
    "legal opinion or a determination of patentability, novelty, FTO, "
    "ownership, infringement risk, efficacy, safety, or clinical value."
)
START_GUIDANCE = (
    "Start with the guided example workflow to see how each evidence stage "
    "turns generated SMILES into transparent research-triage outputs, then "
    "evaluate your own molecule set."
)
ABOUT_WORKFLOW_SECTIONS = (
    ...
    (
        ...
        "This stage organizes semantic evidence similarity for review and "
        "hypothesis generation; it does not establish biological activity.",
    ),
    (
...
WORKFLOW_STEP_PARAGRAPHS = (
    ...
    "only. This step also writes separate Biopharma Analytics artifacts for "
    "evidence readiness, translational positioning, synthetic OMOP-style "
    ...
    "context, and conceptual endpoint mapping for the selected molecule set "
    "and target context. The expected outputs are `biopharma_positioning.csv`, "
...
WORKFLOW_STEP_REFERENCES = (
    ...
    (
        ("Workflow overview", "README.md#workflow-overview"),
        ("RDKit", "https://www.rdkit.org/"),
        ("PubChem", "https://pubchem.ncbi.nlm.nih.gov/"),
        ("ChEMBL", "https://www.ebi.ac.uk/chembl/"),
        ("SureChEMBL", "https://www.surechembl.org/"),
...
def render_biopharma_analytics_page(output_dir: Path) -> None:
    ...
    st.warning(
        "Research and portfolio analytics only. These outputs are not medical "
        "advice, clinical evidence, efficacy, safety, toxicity, legal advice, "
        "or patentability, novelty, freedom-to-operate, infringement, or "
        "ownership determinations."
    )
    st.caption(
        "Translational positioning outputs for the selected molecule set and "
        "target context. The ADORA2A/xanthine target-specific demo uses "
        "illustrative docking values; placeholder/configurable target runs "
        "should be interpreted as user-provided setup context. Mock OMOP/RWE "
        "content is synthetic demonstration data, not real patient data."
    )
    loaded = load_output_directory(output_dir)
    ...
    st.caption(
        "Shows which computational evidence files are available for each "
        "molecule and where follow-up review is still needed."
    )
    if readiness.empty:
    ...
    st.caption(
        "Summarizes public-database differentiation, reference-neighbor "
        "similarity, biomedical context, patent-associated structure crowding, "
        "and ADMET triage signals."
    )
    ...
    st.caption(
        "Synthetic OMOP-style demonstration only; not patient-level RWE and not "
        "real patient data."
    )
    if mock_rwe.empty:
    ...
    st.caption(
        "Conceptual endpoint mapping for translational framing; not a clinical "
...
def render_model_and_data_sources_page(output_dir: Path) -> None:
    ...
    st.caption(
        "Transparency page for configured biomedical and patent/IP-context "
        "models, fallback behavior, public-source status, and app-managed cache "
        "folders. Model weights and downloaded cache data are not committed to "
        "GitHub."
    )
    st.info(
    ...
    if downloads_enabled:
        ...
        st.info(
            "Model download/cache actions are disabled. Set "
            "ALLOW_LOCAL_MODEL_DOWNLOADS=1 before launching Streamlit only when "
            "you intentionally want the explicit local download button."
        )
